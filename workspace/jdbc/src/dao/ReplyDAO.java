...
//   댓글 전체 조회, 대댓글 전체 조회도 포함
	public ArrayList<ReplyDTO> selectAll() {
		...
		int index = 0; // 순서 숫자가 많을 때 사용하면 편리 
...
//	   댓글 수정
	public void update(ReplyVO replyVO) {
		String query = "UPDATE TBL_REPLY SET REPLY_CONTENT = ?, REPLY_UPDATE_DATE = SYSDATE "
				+ "USER_ID = ?, BOARD_ID = ?, REPLY_GROUP = ?, REPLY_DEPTH = ? " + "WHERE Reply_ID = ?";
		connection = DBConnecter.getConnection();
		try {
			preparedStatement = connection.prepareStatement(query);
			preparedStatement.setString(1, replyVO.getReplyContent());
			preparedStatement.setLong(2, replyVO.getUserId());
			preparedStatement.setLong(3, replyVO.getBoardId());
